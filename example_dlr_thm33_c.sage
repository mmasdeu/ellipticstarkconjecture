...
Lpa =  (matrix([piHord[l1],piHord[p]]) * proj_mat**-1)[0,0]

Lp = Lpa
# P0 = E.heegner_point(-N)

test_formula_display45(Lp, p, E, K)



for D,h in [(D,h) for D,h in [(D,QuadraticField(D,'a').class_number()) for D in range(-1,-1000,-1)] if h % 2 == 1 and ZZ(-D).is_prime() and h > 2]:
    try:
        E = EllipticCurve(str(-D))
        E = EllipticCurve(str(-D)+'b')
    except ValueError:
        continue
    if E.rank() != 1:
        continue
